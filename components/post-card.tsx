...
import { card, cardHover } from "@/components/ui";
import { formatDate, type PostMeta } from "@/lib/blog";
import { blogPath } from "@/lib/routes";

const copy = { pl: { read: "Czytaj", min: "min" }, en: { read: "Read", min: "min" } } as const;

export function PostCard({ post, as: Heading = "h3" }: { post: PostMeta; as?: "h2" | "h3" }) {
  const t = copy[post.lang];
  return (
    <article className={`${card} ${cardHover} group relative flex flex-col p-7`}>
      {post.tags[0] && (<span className="w-fit rounded-full bg-muted px-3 py-1 text-xs font-extrabold uppercase tracking-wider text-brand">{post.tags[0]}</span>)}
      <Heading className="font-display mt-4 text-xl font-bold leading-snug text-header">
      ...
      <div className="mt-5 flex items-center justify-between text-xs text-ink-soft">
        ...
        <span className="flex items-center gap-1 font-bold text-brand">{t.read} <ArrowRight size={14} className="transition-transform group-hover:translate-x-1" aria-hidden="true" /></span>
      </div>
    </article>
  );
}
