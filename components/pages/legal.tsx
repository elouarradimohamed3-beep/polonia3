...
const labels = {
  pl: { updated: "Ostatnia aktualizacja", contact: "Kontakt", company: "Firma", address: "Adres", tax: "NIP / numer podatkowy" },
  en: { updated: "Last updated", contact: "Contact", company: "Company", address: "Address", tax: "Tax ID" },
} as const;

export const legalMetadata = (lang: Lang, key: LegalKey): Metadata =>
  pageMetadata({ lang, title: LEGAL[key][lang].title, description: LEGAL[key][lang].description, paths: { ...ROUTES[key as RouteKey] } });
      ...
      <Section>
        <article className="mx-auto max-w-3xl">
          ...
          <div className="mt-8 space-y-8">
            {doc.sections.map((s) => (
              <section key={s.h}>
                <h2 className="font-display text-xl font-bold text-header">{s.h}</h2>
                <div className="mt-3 space-y-3 leading-relaxed text-ink-soft">
                  {s.p?.map((x) => <p key={x}>{x}</p>)}
                  {s.ul && (<ul className="list-disc space-y-1.5 pl-6 marker:text-brand">{s.ul.map((x) => <li key={x}>{x}</li>)}</ul>)}
                </div>
              </section>
            ))}
          </div>
