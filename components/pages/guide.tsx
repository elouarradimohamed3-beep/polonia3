...
export const guideMetadata = (lang: Lang): Metadata =>
  pageMetadata({ lang, title: GUIDE_TEXT[lang].title, description: GUIDE_TEXT[lang].description, paths: { ...ROUTES.guide } });

function Steps({ steps }: { steps: string[] }) {
  return (
    <ol className="list-decimal space-y-1.5 pl-6 leading-relaxed text-header marker:font-bold marker:text-brand">
...
export function GuidePage({ lang }: { lang: Lang }) {
  ...
  const howTo = sections.map((s) => {
    const steps = s.steps ?? s.groups?.flatMap((grp) => grp.steps.map((x) => `${grp.heading}: ${x}`)) ?? [];
    return { "@type": "HowTo", "@id": `${abs(p)}#${s.id}`, name: s.title, inLanguage: lang, step: steps.map((text, i) => ({ "@type": "HowToStep", position: i + 1, name: `${g.stepWord} ${i + 1}`, text })) };
  });
  return (
    <>
      ...
      <Section>
        ...
        <nav aria-label={g.h1} className="mx-auto mb-10 flex max-w-4xl flex-wrap justify-center gap-2">
          {sections.map((s) => (
            <a key={s.id} href={`#${s.id}`} className="rounded-full border border-border bg-white px-4 py-1.5 text-sm font-semibold text-header hover:border-brand hover:text-brand">{s.title}</a>
          ))}
        </nav>
        <div className="mx-auto max-w-3xl space-y-5">
          {sections.map((s, i) => (
            <article key={s.id} id={s.id} className={`${card} scroll-mt-28 p-6 md:p-8`}>
              ...
              {s.steps && (<div className="mt-4"><Steps steps={s.steps} /></div>)}
              {s.note && (<p className="mt-4 rounded-lg bg-muted p-3 text-sm text-header"><strong>{g.note}:</strong> {s.note}</p>)}
        ...
        <div className="mt-12 text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <a href={whatsappLink(g.helpMsg)} target="_blank" rel="noopener noreferrer" className="btn btn-primary">{g.help}</a>
            <Link href={sectionHref(lang, "trial")} className="btn btn-ghost">{g.trial}</Link>
          </div>
        </div>
      </Section>
    </>
