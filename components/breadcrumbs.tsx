...
export function Breadcrumbs({ lang, trail }: { lang: Lang; trail: { name: string; path: string }[] }) {
  ...
  return (
    <>
      <nav aria-label={lang === "pl" ? "Ścieżka nawigacji" : "Breadcrumb"} className="border-b border-border bg-white/60">
          ...
          {full.map((item, i) => {
            ...
            return (
              <li key={item.path} className="flex items-center gap-1.5">
                {last ? (
                  <span aria-current="page" className="font-semibold text-header">{item.name}</span>
                ) : (
                  <>
                    <Link href={item.path} className="text-brand hover:underline">{item.name}</Link>
